 ...
 */
const activateZFCPController = (controllerId: string) =>
  post(`/api/storage/zfcp/controllers/${controllerId}/activate`);

/**
 * Returns list of WWPNs for given controller
 */
const fetchWWPNs = (controllerId: string): Promise<string[]> =>
  get(`/api/storage/zfcp/controllers/${controllerId}/wwpns`);

/**
 * Returns list of LUNs for give controller and WWPN
 */
const fetchLUNs = (controllerId: string, wwpn: string): Promise<string[]> =>
  get(`/api/storage/zfcp/controllers/${controllerId}/wwpns/${wwpn}/luns`);

/**
 * Actives disk on given controller with WWPN and LUN
 */
const activateZFCPDisk = (controllerId: string, wwpn: string, lun: string) =>
  post(`/api/storage/zfcp/controllers/${controllerId}/wwpns/${wwpn}/luns/${lun}/activate_disk`);

/**
 * Deactives disk on given controller with WWPN and LUN
 */
const deactivateZFCPDisk = (controllerId: string, wwpn: string, lun: string) =>
  post(`/api/storage/zfcp/controllers/${controllerId}/wwpns/${wwpn}/luns/${lun}/deactivate_disk`);
