...
.agm-plan-list {
  ...
  overflow-x: auto;
  /* What the rows measure themselves against. The list is a column of a page
     that may be sharing the width with the sheet, so the window says nothing
     about how much room a row has. */
  container-type: inline-size;
  container-name: list;
}

/* Too narrow for four columns, and a table that scrolls takes the cost out of
   sight, which is the one thing on a row a reader must not miss. So the row
   stops being four cells across and becomes two lines: what the device is and
   what the plan costs on the first, what it is being used for on the second.

   Nothing is hidden and nothing scrolls. It buys the width back from the
   purpose, which is the column a reader is least likely to be scanning for and
   the one the sheet repeats in full. */
@container list (max-width: 40rem) {
  .agm-plan-table {
    min-width: 0;
  }

  /* The column names no longer sit over columns. Every cell keeps its own name
     for a screen reader through data-label, which is why they can go. */
  .agm-plan-devices thead {
    position: absolute;
    width: 1px;
    height: 1px;
    overflow: hidden;
  ...
  /* The row draws its own frame. A table puts the rule under each cell, and a
     row laid out as a grid turns that into a run of broken lines with the
     second line stranded below the last of them. */
  .agm-plan-row {
    display: grid;
  ...
  .agm-plan-row > th[scope="row"] {
    grid-area: device;
  }

  .agm-plan-row > td:nth-of-type(1) {
    grid-area: purpose;
    font-size: var(--pf-t--global--font--size--xs);
    color: var(--pf-t--global--text--color--subtle);
  }

  .agm-plan-row > td:nth-of-type(2) {
    grid-area: changes;
  }

  .agm-plan-row > .agm-plan-row-control {
    grid-area: control;
    align-self: start;
  }

  /* The parts of the purpose run on one line: two lines of it under one line of
     identity stops reading as a note about that identity. */
  .agm-plan-row > td:nth-of-type(1) > div {
    display: inline;
  }

  .agm-plan-row > td:nth-of-type(1) > div + div::before {
    content: " · ";
  }
}
