...
import { Si7zip } from "@icons-pack/react-simple-icons";
    ...
    const loadNetworks = async () => {
      ...
      return accessPoints
        ...
        .reduce((networks, ap) => {
          ...
          // Group networks
          if (network.device) {
            if (network.device.state === DeviceState.FAILED) {
              network.error = "Failed";
            }
            if (network.device.state === DeviceState.NEEDAUTH) {
              network.error = "Need authentication";
            }
        ...
        case NetworkEventTypes.DEVICE_UPDATED: {
          ...
          if (data.state === DeviceState.FAILED) {
            ...
            if (current_device && (data.stateReason === 7)) {
              console.log("NEEDAUTH ", current_device.connection);
              setNeedAuth(current_device.connection);
            }
          }
          if (data.state === DeviceState.NEEDAUTH) {
            selectedNetwork.error = "Need authentication";
          }
          setDevices(devs => {
