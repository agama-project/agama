...
impl security::Callback for Security {
    fn unsigned_file(&self, file: String, repository_alias: String) -> bool {
        ...
        let text = if repository_alias.is_empty() {
            format!("The file {file} is not digitally signed. The origin \
                and integrity of the file cannot be verified. Use it anyway?")
        } else {
            format!("The file {file} from {repository_alias} is not digitally signed. The origin \
                and integrity of the file cannot be verified. Use it anyway?")
        };
    ...
    fn accept_key(
        ...
        let actions = [
            ...
            ("Skip", labels[1].as_str()),
        ];
        let data = [("id", key_id.as_str()), ("name", key_name.as_str()), ("fingerprint", key_fingerprint.as_str())];
    ...
    fn unknown_key(&self, file: String, key_id: String, repository_alias: String) -> bool {
        // TODO: localization for text when parameters in gextext will be solved
        let text = if repository_alias.is_empty() {
            format!("The file {file} is digitally signed with \
                the following unknown GnuPG key: {key_id}. Use it anyway?")
        } else {
            format!("The file {file} from {repository_alias} is digitally signed with \
                the following unknown GnuPG key: {key_id}. Use it anyway?")
        };
        let labels = [gettext("Yes"), gettext("No")];
        let actions = [
    ...
    fn verification_failed(
            ...
            // TODO: localization for text when parameters in gextext will be solved
        let text = if repository_alias.is_empty() {
            format!("The file {file} is digitally signed with the \
            ...
            format!("The file {file} from {repository_alias} is digitally signed with the \
                following GnuPG key, but the integrity check failed: {key_id} ({key_name}). \
                Use it anyway?")
        };
        let labels = [gettext("Yes"), gettext("No")];
    ...
    fn checksum_missing(&self, file: String) -> bool {
                    // TODO: localization for text when parameters in gextext will be solved
        let text = format!("No checksum for the file {file} was found in the repository. This means that \
    ...
    fn checksum_unknown(&self, file: String, checksum: String) -> bool {
        let text = format!("The checksum of the file {file} is \"{checksum}\" but the expected checksum is \
    ...
    fn checksum_wrong(&self, file: String, expected: String, actual: String) -> bool {
                let text = format!("The expected checksum of file %{file} is \"%{actual}\" but it was expected to be \
