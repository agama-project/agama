...
impl security::Callback for Security {
    fn unsigned_file(&self, file: String, repository_alias: String) -> bool {
        tracing::info!(
            "unsigned_file callback triggered for file: {}, repo: {}",
            file,
            repository_alias
        );
    ...
    fn accept_key(
        ...
        tracing::info!(
            "accept_key callback triggered for key_id: {}, key_name: {}, key_fingerprint: {}",
            key_id,
            key_name,
            key_fingerprint
        );
    ...
    fn unknown_key(&self, file: String, key_id: String, repository_alias: String) -> bool {
        tracing::info!(
            "unknown_key callback triggered for file: {}, key_id: {}, repo: {}",
            file,
            key_id,
            repository_alias
        );
    ...
    fn verification_failed(
        ...
        tracing::info!("verification_failed callback triggered for file: {}, key_id: {}, key_name: {}, repo: {}", file, key_id, key_name, repository_alias);
        // TODO: localization for text when parameters in gextext will be solved
        let text = if repository_alias.is_empty() {
            format!(
    ...
    fn checksum_missing(&self, file: String) -> bool {
        tracing::info!("checksum_missing callback triggered for file: {}", file);
        // TODO: localization for text when parameters in gextext will be solved
        let text = format!(
    ...
    fn checksum_unknown(&self, file: String, checksum: String) -> bool {
        tracing::info!(
            "checksum_unknown callback triggered for file: {}, checksum: {}",
            file,
            checksum
        );
        let text = format!(
    ...
    fn checksum_wrong(&self, file: String, expected: String, actual: String) -> bool {
        tracing::info!(
            "checksum_wrong callback triggered for file: {}, expected: {}, actual: {}",
            file,
            expected,
            actual
        );
