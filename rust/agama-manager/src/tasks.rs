...
use crate::{
    bootloader, files, hostname, iscsi, l10n, proxy, security, service, software, storage, users,
};
    ...
    async fn dispatch(&mut self, task: Task) {
        match task {
            ...
            Task::SetConfig { product, config } => {
                ...
                if let Err(error) = action.run(product.clone(), config).await {
