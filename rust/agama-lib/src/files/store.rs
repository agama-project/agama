...
impl FilesStore {
    ...
    pub async fn load(&self) -> Result<FilesSettings, ServiceError> {
        self.files_client.get_files().await
    }

    pub async fn store(&self, settings: &FilesSettings) -> Result<(), ServiceError> {
        self.files_client.set_files(settings).await?;
        Ok(())
    }
}
