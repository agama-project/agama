...
impl TryFrom<NetworkSettings> for NetworkState {
    type Error = NetworkStateError;

    fn try_from(settings: NetworkSettings) -> Result<Self, Self::Error> {
        let mut connections: Vec<Connection> = Vec::with_capacity(settings.connections.0.len());

        for conn in settings.connections.0 {
            let connection = Connection::try_from(conn.clone())?;
            connections.push(connection);

            if let Some(bond) = &conn.bond {
                dbg!("Bond with: ", &bond.ports);
            }
            if let Some(bridge) = &conn.bridge {
                dbg!("Bridge with: ", &bridge.ports);
            }
        }
