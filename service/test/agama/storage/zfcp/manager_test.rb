...
describe Agama::Storage::ZFCP::Manager do
  ...
  describe "#deactivate_controller" do
    before do
      allow(yast_zfcp).to receive(:deactivate_controller).and_return(output)
    ...
    it "tries to deactivate the controller with the given channel id" do
      expect(yast_zfcp).to receive(:deactivate_controller).with("0.0.fa00")

      subject.deactivate_controller("0.0.fa00")
    end

    it "returns the exit code" do
      result = subject.deactivate_controller("0.0.fa00")

      expect(result).to eq(3)
    end

    context "if the controller was correctly deactivated" do
      let(:output) { { "exit" => 0 } }
    ...
    context "if the controller was not deactivated" do
      let(:output) { { "exit" => 1 } }
