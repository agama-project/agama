...
describe Agama::Storage::ConfigJSONSolver do
  ...
  describe "#solve" do
    ...
    context "if a drive generates partitions" do
      ...
      context "if a 'volumeGroups' config already specifies any of the paths" do
        let(:default_paths) { ["/", "swap", "/home"] }
        let(:mandatory_paths) { ["/", "swap"] }
        ...
        it "only adds 'partitions' for the the missing paths" do
          subject.solve(config_json)
          partitions_json = config_json[:drives][0][:partitions]

          expect(partitions_json).to contain_exactly(
            { filesystem: { path: "/" } },
            { filesystem: { path: "/home" } }
          )
        end
    ...
    context "if a volume group generates logical volumes" do
      ...
      context "if a 'drives' config specifies a partition with 'generate'" do
        let(:config_json) do
          {
            drives:       [
              {
