...
describe Agama::DBus::Storage::Manager do
  ...
  describe "#proposal_json" do
    context "if no proposal has been successfully calculated" do
      ...
      it "returns 'null'" do
        expect(subject.proposal_json).to eq("null")
      end
    end
      ...
      describe "proposal[:actions]" do
        ...
        context "if there are no actions" do
          ...
          it "returns an empty list" do
            expect(parse(subject.proposal_json)[:actions]).to eq([])
          end
        end
        ...
        context "if there are actions" do
          ...
          it "returns a list with a hash for each action" do
            all_actions = parse(subject.proposal_json)[:actions]
  ...
  describe "#system_json" do
    context "if the system has not been probed yet" do
      ...
      it "returns 'null'" do
        expect(subject.system_json).to eq("null")
      end
    end
    ...
    describe "system_json[:availableDrives]" do
      context "if there is no available drives" do
        let(:available_drives) { [] }

        it "returns an empty list" do
          expect(parse(subject.system_json)[:availableDrives]).to eq([])
        end
      end
      ...
      context "if there are available drives" do
        ...
        it "retuns the id of each drive" do
          result = parse(subject.system_json)[:availableDrives]
          expect(result).to contain_exactly(95, 96, 97)
        end
      end
    end

    describe "system_json[:candidateDrives]" do
      context "if there is no candidate drives" do
        let(:candidate_drives) { [] }

        it "returns an empty list" do
          expect(parse(subject.system_json)[:candidateDrives]).to eq([])
        end
      end
      ...
      context "if there are candidate drives" do
        ...
        it "retuns the id of each drive" do
          result = parse(subject.system_json)[:candidateDrives]
          expect(result).to contain_exactly(95, 96)
        end
      end
    end

    describe "system_json[:availableMdRaids]" do
      context "if there is no available MD RAIDs" do
        let(:available_raids) { [] }

        it "returns an empty list" do
          expect(parse(subject.system_json)[:availableMdRaids]).to eq([])
        end
      end
      ...
      context "if there are available MD RAIDs" do
        ...
        it "returns the id of each MD RAID" do
          result = parse(subject.system_json)[:availableMdRaids]
          expect(result).to contain_exactly(100, 101, 102)
        end
      end
    end

    describe "system_json[:candidateMdRaids]" do
      context "if there is no candidate MD RAIDs" do
        let(:candidate_raids) { [] }

        it "returns an empty list" do
          expect(parse(subject.system_json)[:candidateMdRaids]).to eq([])
        end
      end
      ...
      context "if there are candidate MD RAIDs" do
        ...
        it "retuns the path of each MD RAID" do
          result = parse(subject.system_json)[:candidateMdRaids]
          expect(result).to contain_exactly(100, 101)
        end
      end
    end

    describe "system_json[:issues]" do
      context "if there is no candidate drives" do
        let(:candidate_drives) { [] }

        it "contains a issue about the absence of disks" do
          result = parse(subject.system_json)[:issues]
    ...
    describe "system_json[:productMountPoints]" do
      before do
      ...
      context "with no storage section in the configuration" do
        ...
        it "contains an empty list" do
          expect(parse(subject.system_json)[:productMountPoints]).to eq([])
        end
      end
      ...
      context "with a set of volume templates in the configuration" do
        ...
        it "contains the mount points of each volume template" do
          result = parse(subject.system_json)
          expect(result[:productMountPoints]).to contain_exactly("/", "swap", "/home")
        end
      end
    end

    describe "system_json[:volumeTemplates]" do
      before do
        backend.update_product_config(product_config)
      end
      ...
      context "with no storage section in the configuration" do
        ...
        it "contains only a generic default template with empty path" do
          ...
          templates = parse(subject.system_json)[:volumeTemplates]
    ...
    context "if no storage configuration is given" do
      ...
      context "when storage devices are already activated and probed" do
        before do
          ...
          # Generate system according to the current product config.
          subject.system_json = subject.send(:generate_system_json)
        end
  ...
  describe "#config_json" do
    context "if a proposal has not been calculated" do
      it "returns 'null'" do
        expect(subject.config_json).to eq("null")
      end
    end
    ...
    context "if an agama proposal has been calculated" do
      ...
      it "returns serialized storage config" do
        expect(subject.config_json).to eq(serialize(config_json))
      end
    end
    ...
    context "if an AutoYaST proposal has been calculated" do
      ...
      it "returns the serialized AutoYaST config" do
        expect(subject.config_json).to eq(serialize(autoyast_json))
      end
    end
  end
  ...
  describe "#config_model_json" do
    context "if a proposal has not been calculated" do
      it "returns 'null'" do
        expect(subject.config_model_json).to eq("null")
      end
    end
    ...
    context "if an agama proposal has been calculated" do
      ...
      it "returns the serialized config model" do
        expect(subject.config_model_json).to eq(
          serialize({
            boot:         {
              configure: true,
              device:    {
  ...
  describe "#probe" do
    before do
      ...
      # Initializes system_json to mimic the system is not probed yet.
      subject.system_json = "null"
    end
  ...
  describe "#issues_json" do
    context "if no proposal has been calculated" do
      it "returns an empty array" do
        expect(subject.issues_json).to eq "[]"
      end
    end
    ...
    context "if an agama proposal failed to be calculated" do
      ...
      it "returns the list of proposal issues" do
        result = parse(subject.issues_json)
