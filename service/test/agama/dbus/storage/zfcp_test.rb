...
RSpec.describe Agama::DBus::Storage::ZFCP do
  ...
  before do
    ...
    allow(manager).to receive(:allow_lun_scan?).and_return(allow_lun_scan)
    allow(manager).to receive(:controllers).and_return(controllers)
    allow(manager).to receive(:devices).and_return(devices)
  ...
  describe "#serialized_issues" do
    ...
    it "returns the issues as a JSON" do
      ...
      expect(subject.serialized_issues).to eq(expected_value)
    end
    ...
    context "when there are no issues" do
      ...
      it "returns an empty JSON array" do
        expect(subject.serialized_issues).to eq(JSON.pretty_generate([]))
      end
    end
  end
  ...
  describe "#configure" do
    before do
      allow(manager).to receive(:configured?).and_return(false)
    end
    ...
    context "if the system was already configured" do
      before do
        allow(manager).to receive(:configured?).and_return(true)
      end

      it "does not configure the system again" do
