  ...
  module Software
    ...
    class Proposal
      ...
      def packages_download_count
        ...
        remote_repos = Agama::Software::Repository.all.select(&:enabled?).reject(&:local?).map(&:repo_id)
        logger.info "Remote repositories: #{remote_repos.inspect}"

        # shortcut, no remote repository
        return 0 if remote_repos.empty?

        # count the packages to install from each medium,
        ...
        media.each_with_index do |medium, index|
          ...
          if remote_repos.include?(src_id)
            download += counts[index].sum
          end
        end
