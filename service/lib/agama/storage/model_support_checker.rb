...
module Agama
  module Storage
    ...
    class ModelSupportChecker
      ...
      # @note A solved config is expected. Otherwise some checks cannot be done.
      #
      # @return [Booelan]
      def supported?
        return @supported unless @supported.nil?

        unsupported = any_unsupported_device? ||
    ...
    private
      ...
      def reused_partition?(partition_config)
        !new_partition(partition_config) && !config.search.skip_device?
      end
